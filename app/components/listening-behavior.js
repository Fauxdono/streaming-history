...
import { computeBehaviorStats } from './streaming/behavior.js';
...
const filteredData = useMemo(() => {
  ...
  // Behavior is the landing subtab, so this always runs on mount; keyed only
  // on the data so subtab switches and theme changes don't discard it. The
  // single-pass aggregate lives in streaming/behavior.js.
  const behaviorStats = useMemo(() => computeBehaviorStats(filteredData), [filteredData]);

  // Theme-dependent pie slices layered on top — a theme change only re-colors
  const behaviorData = useMemo(() => {
    const shuffle2 = chart.ramp(2);
    const shuffleData = [
      { name: 'Shuffle On', value: behaviorStats.shufflePlays, color: shuffle2[0] },
      { name: 'Shuffle Off', value: behaviorStats.normalPlays, color: shuffle2[1] }
    ];
    const skip3 = chart.ramp(3);
    const skipData = [
      { name: 'Completed', value: behaviorStats.completedTracks, color: skip3[0] },
      { name: 'Skipped', value: behaviorStats.skippedTracks, color: skip3[1] },
      { name: 'Other End', value: behaviorStats.totalTracks - behaviorStats.completedTracks - behaviorStats.skippedTracks, color: skip3[2] }
    ];
    return { ...behaviorStats, shuffleData, skipData };
  }, [behaviorStats, chart]);
