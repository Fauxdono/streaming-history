...
const AlbumCard = ({ album, index, processedData, formatDuration, textTheme = 'cyan', backgroundTheme = 'cyan' }) => {
  ...
  const getThemedColors = () => {
    ...
    const backgroundColors = {
      ...
      green: {
        bg: isDarkMode ? 'bg-black' : 'bg-white', border: isDarkMode ? 'border-green-200' : 'border-green-200',
      ...
      amber: {
        bg: isDarkMode ? 'bg-black' : 'bg-white', border: isDarkMode ? 'border-amber-200' : 'border-amber-200',
      ...
      orange: {
        bg: isDarkMode ? 'bg-black' : 'bg-white', border: isDarkMode ? 'border-orange-200' : 'border-orange-200',
      ...
      red: {
        bg: isDarkMode ? 'bg-black' : 'bg-white', border: isDarkMode ? 'border-red-200' : 'border-red-200',
      ...
      indigo: {
        bg: isDarkMode ? 'bg-black' : 'bg-white', border: isDarkMode ? 'border-indigo-200' : 'border-indigo-200',
      ...
      emerald: {
        bg: isDarkMode ? 'bg-black' : 'bg-white', border: isDarkMode ? 'border-emerald-200' : 'border-emerald-200',
  ...
  return (
    <div className={`p-3 ${colors.bg} rounded shadow-sm ${isDarkMode ? 'border border-gray-700' : `border ${colors.border}`} hover:${colors.borderHover} transition-all duration-300 relative`}>
      ...
      {/* Divider */}
      <div className={`${isDarkMode ? 'border-t border-gray-700' : `border-t ${colors.border}`} my-2`}></div>
      ...
      {showTracks && otherTracks.length > 0 && (
        <div className={`mt-1 max-h-64 overflow-y-auto text-xs ${isDarkMode ? 'border border-gray-700' : `border ${colors.border}`} rounded`}>
