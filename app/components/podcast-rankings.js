...
const PodcastRankings = ({
  ...
  // Color system for colorful/minimal modes - colorful has contrast, minimal is flat (Red theme)
  const modeColors = isColorful ? {
    ...
    bg: isDarkMode ? 'bg-red-900' : 'bg-red-200',
    bgLight: isDarkMode ? 'bg-red-800' : 'bg-red-100',
    bgCard: isDarkMode ? 'bg-red-800' : 'bg-red-100',
    ...
    buttonActive: isDarkMode ? 'bg-black text-white border border-white translate-x-[2px] translate-y-[2px] shadow-[inset_-2px_-2px_0_0_white]' : 'bg-white text-black border border-black translate-x-[2px] translate-y-[2px] shadow-[inset_-2px_-2px_0_0_black]',
    ...
    if (isMobile) {
      ...
      return (
        <tr key={episode.key} className={`border-b hover:opacity-80 ${modeColors.border}`}>
          <td className={`p-2 ${modeColors.text}`}>
            <div className="flex flex-col">
              <div className="flex items-center">
                <span className={`font-bold text-xs mr-2 ${modeColors.text}`}>{index + 1}.</span>
                <div className="font-medium text-sm">{episode.episodeName}</div>
              </div>
              <div
                className={`text-xs cursor-pointer hover:underline ${modeColors.textLight}`}
          ...
          <td className={`p-2 align-top text-right ${modeColors.text}`}>
            <div className="flex flex-col">
