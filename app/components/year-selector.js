...
const YearSelector = ({
  ...
  const [rangeTapMode, setRangeTapMode] = useState(() => {
    if (typeof window === 'undefined') return true;
    const stored = localStorage.getItem('yearSelectorRangeTapMode');
    return stored === null ? true : stored === 'true';
  });
    ...
    if (mode === 'single') {
      if (isHorizontal && !isMobilePortraitHz) {
        if (isMobile && isLandscape) {
          ...
          // 3-col months → 4 rows; 7-col days → 5 rows
          if (showMonthSelector) h = Math.max(h, 4 * 26 + 12); // ~116px
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('yearSelectorRangeTapMode', String(rangeTapMode));
    }
  }, [rangeTapMode]);
  ...
  const renderDayGrid = (daysArray, selectedValue, onSelect, cols, wrap = false) => (
            ...
            <>
              ...
              {yearRange.startYear && yearRange.endYear && !(isMobile && !isLandscape && isHorizontal) && (
                <>
                  ...
                  {/* Split mode, horizontal: SY/EY year rows + SM/SD + EM/ED rows */}
                  {!rangeTapMode && isHorizontal && (
